...
export function adminToken() {
  if (!process.env.ADMIN_SESSION_SECRET) throw new Error("Admin secret is not configured");
  return createHmac("sha256", process.env.ADMIN_SESSION_SECRET).update("royalarm-admin-session-v1").digest("hex");
}
